...
import Image1 from "../../yay.jpeg";
import Image2 from "../../Untitled design.jpg";
		...
		<section className="patient-bar">
			...
			<select id="patient-select" className="patient-select">
				{Array.from({ length: 10 }, (_, i) => (
					<option key={i} value={`patient${i + 1}`}>{`Patient ${
						i + 1
					}`}</option>
				))}
			</select>
		...
		<section className="summary-box">
			...
			<div className="summary-content">
				<div
					...
					onClick={() => alert("Genomic Data clicked")}
				>
					Genomic Data
				</div>
				<div
					className="summary-icon"
					onClick={() => alert("Patient Charts clicked")}
				>
					Patient Charts
				</div>
				<div
					className="summary-icon"
					onClick={() => alert("Family Health History clicked")}
				>
